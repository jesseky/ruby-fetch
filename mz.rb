...
class FetchLite
  ...
  def initialize(opt={})
    ...
    @open_opt = {'User-Agent'=>@agent, :allow_redirections=>:safe}.merge(opt[:open_opt] || {})
  end
  ...
  def archive(url, opt)
    ...
    begin
      mon = Nokogiri::HTML.parse(open(url, @open_opt)).css(opt[:month_query])
    rescue => e
      abort "url: #{url}, error: #{e.message}"
    end
    Parallel.each(mon, :in_threads=>[opt[:threads] || 10, mon.size].min) do |m|
      begin 
        ...
        v = open(opt[:list_url] % URI.encode(u, /\W/), @open_opt).read
